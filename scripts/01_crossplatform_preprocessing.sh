...
while getopts "b:i:g:j:o:q:s:t:w:" opt; do
  case $opt in
...
echo $(stm "START scATAC QC Preprocessing")
...
echo $(check_param "-q" "Generate QC Metrics Only" ${qc_only})
...
if [ -f ${in_singlecell} ]; then
  echo $(stm "Generating barcode lists from $in_singlecell")

  bc_col=barcode
  pf_col=passed_filters
  ...
  cat ${in_singlecell} \
    | awk -F',' -v OFS='\t' \
      ...
      -v b=${bc_col} \
      -v p=${pf_col} \
      '{ if(NR == 1) {for (i = i;i<=NF;i++) {f[$i] = i} } else if($f[p] > 1000 && NR > 2) { s=substr($f[b],0,4); print $f[b] > a s c } }'
  ...
  echo $(stm "Generating barcode list from $in_singlecell")
...
echo $(stm "$(elt $split_start_time $total_start_time)" )
echo $(stm "END scATAC QC Preprocessing")

...
while getopts "b:i:g:j:m:o:t:s:" opt; do
  case $opt in
...
if [ ! -d ${output_dir} ]; then
  echo $(stm "Making Output Directory ${output_dir}")
  mkdir -p ${output_dir}
fi
...
# Generate barcode lists for each prefix

mkdir ${temp_dir}/bclist

echo $(stm "Generating barcode list from ${in_meta}")
...
| awk -F',' -v OFS='\t' \
  ...
  -v c=.tsv \
  -v b=${barcode_col} \
  '{ if(NR == 1) {for (i = i;i<=NF;i++) {f[$i] = i} } else { s=substr($f[b],0,4); print $f[b] > a s c } }'
...
prefixes="$(ls ${temp_dir}/bclist | sed 's/[0-9]\+_//g' | sed 's/.tsv//g' | sort | uniq)"

mkdir ${temp_dir}/frag_sort/
parallel -j ${num_jobs} \
  --bar \
  "cat ${temp_dir}/bclist/{}.tsv \
    ${temp_dir}/frag/*_{}.bed \
  | awk -F'\t' -v OFS='\t' \
...
parallel -j ${num_jobs} \
  ...
  > ${output_dir}/${sample_id}_{}_sparse_matrix.tsv" \
  ::: ${ref_names}
...
#   > ${output_dir}/${sample_id}_{}_total_counts.tsv" \
#   ::: $ref_names
...
split_start_time="$(date -u +%s)"
  ...
  | awk -F'\t' -v OFS='\t' \
    '{print $1,$2,$5,$6}' \
  > ${output_dir}/${sample_id}_window_5k_counts.tsv
...
#   -c 2 \
#   -o sum \
# > ${output_dir}/${sample_id}_saturation_curve.tsv
# 
# echo $(stm "$(elt $split_start_time $total_start_time)" )
# split_start_time="$(date -u +%s)"
# 
...
#   > ${output_dir}/${sample_id}_fragment_widths.tsv
...
echo $(stm "$(elt $split_start_time $total_start_time)" )
echo $(stm "END scATAC QC Postprocessing")
